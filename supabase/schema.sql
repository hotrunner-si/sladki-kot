...
create table public.delivery_items (
  id uuid primary key default gen_random_uuid(), delivery_id uuid references public.deliveries(id) on delete cascade,
  product_id uuid not null references public.products(id) on delete restrict, quantity numeric(12,3) not null check(quantity > 0),
...
create index delivery_items_received_idx on public.delivery_items(received_at desc) where received_at is not null;
