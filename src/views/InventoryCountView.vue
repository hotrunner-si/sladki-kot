...
const products=ref([]),quantities=ref({}),modes=ref({}),draftId=ref(null),hasDraft=ref(false),loading=ref(true),saving=ref(false),error=ref(''),message=ref(''),started=ref(false),search=ref(''),closed=ref({}),showDiscardConfirm=ref(false),showIncompleteConfirm=ref(false),showNewConfirm=ref(false)
const {state}=useAuth(),router=useRouter()
let saveTimer, presenceTimer
...
async function loadDraft() {
  ...
  const { data, error: draftError } = await supabase.from('inventory_counts').select('id,status').eq('last_edited_by',state.user.id).in('status',['draft','incomplete']).order('updated_at',{ascending:false}).limit(1).maybeSingle()
  ...
  if(!items.length){if(data.status==='incomplete')await supabase.from('inventory_counts').update({status:'draft',last_edited_by:state.user.id}).eq('id',data.id);await supabase.from('inventory_counts').delete().eq('id',data.id);return}
...
async function requestNewCount(){
  ...
  try{const cutoff=new Date(Date.now()-36*60*60*1000).toISOString();const{data,error:e}=await supabase.from('inventory_counts').select('id').eq('status','incomplete').gte('updated_at',cutoff).limit(1);if(e)throw e;if(hasDraft.value||data.length){showNewConfirm.value=true;return}await beginFreshCount()}catch{error.value='Preverjanje nezaključenih popisov ni uspelo.'}
}
async function beginFreshCount(){
  clearTimeout(saveTimer);clearInterval(presenceTimer)
  if(draftId.value)await supabase.from('inventory_counts').update({status:'incomplete',active_until:null}).eq('id',draftId.value).eq('status','draft')
  draftId.value=null;hasDraft.value=false;quantities.value={};showNewConfirm.value=false;started.value=true
}
function continueOldCount(){showNewConfirm.value=false;router.push('/counts')}

async function ensureDraft(){if(draftId.value)return;if(!hasEntries.value)return;const id=crypto.randomUUID(),now=new Date(),activeUntil=new Date(now.getTime()+60000).toISOString();const{error:e}=await supabase.from('inventory_counts').insert({id,created_by:state.user.id,last_edited_by:state.user.id,status:'draft',active_until:activeUntil});if(e)throw e;draftId.value=id;hasDraft.value=true;startPresence()}
...
function scheduleSave(){if(!started.value||!hasEntries.value)return;clearTimeout(saveTimer);saveTimer=setTimeout(()=>saveDraft().catch(()=>{error.value='Sprotno shranjevanje ni uspelo.'}),450)}
watch(quantities,scheduleSave,{deep:true})

async function discard(){saving.value=true;error.value='';try{if(draftId.value){const {error:e}=await supabase.from('inventory_counts').delete().eq('id',draftId.value);if(e)throw e}clearTimeout(saveTimer);clearInterval(presenceTimer);draftId.value=null;hasDraft.value=false;quantities.value={};started.value=false;showDiscardConfirm.value=false;message.value='Popis je zavržen.'}catch{error.value='Popisa ni bilo mogoče zavreči.'}finally{saving.value=false}}
